...
Instrumented with timing prints and per-day checkpointing so a slow or
interrupted run can be diagnosed and resumed rather than rerun blind.
"""
...
def load_tweets() -> pd.DataFrame:
    ...
    print(f"Downloaded {len(df):,} rows in {time.time()-t1:.1f}s "
          f"({df['trading_day'].nunique()} trading days, "
          f"{df['is_near_duplicate'].sum():,} flagged near-duplicate)", flush=True)
    return df
...
def compute_graph_features(day: str, G: nx.DiGraph) -> dict:
    ...
    if n_nodes < 3:
        return {'trading_day': day, 'n_nodes': n_nodes, 'n_edges': n_edges,
...
def main():
    ...
    # Resume support: skip days already checkpointed
    try:
